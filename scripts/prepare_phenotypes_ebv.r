...
- Read in the data available for EBV
...
How does OD compare against std_OD?
  ...
  ```{r}
plot(ebv_od_F7 ~ ebv_std_F7, data=b1)
```
...
How much similarity is there across age groups?
  ...
  ```{r}
plot(ebv_od_F7 ~ ebv_od_F11, data=b1)
plot(ebv_od_F7 ~ ebv_std_TF3, data=b1)
plot(ebv_od_F11 ~ ebv_std_TF3, data=b1)
...
There are three time points for children with EBV measures... How many from each time point?
...
for (i in 1:nrow(b1)) {
  ...
  # frequency of visits
  res4 = 0
  for (j in c("ebv_od_F7","ebv_od_F11","ebv_od_TF3")) {
    if (b1[i,j]>0) {
      res1 = b1[i,j]
      res4=res4+1
      if (j=="ebv_od_F7") {
        res2=7
        res3=b1[i,"ebv_std_F7"]
      }
      if (j=="ebv_od_F11") {
        res2=11
        res3=b1[i,"ebv_std_F11"]
      }
      if (j=="ebv_od_TF3") {
        res2=15
        res3=b1[i,"ebv_std_TF3"]
      }
    }
  }
  t[i]=res1
...
How many records for each time point?
  ...
  ```{r}
nrow(c[c$ebv_od_F7>-1,])
nrow(c[c$ebv_od_F11>-1,])
nrow(c[c$ebv_od_TF3>-1,])
